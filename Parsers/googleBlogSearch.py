...
def blogSearch((searchTerm, start_month, start_year, end_month, end_year)):
    ...
    requests = {domain: SEARCH_URL.substitute(params, site=domain) for domain in sitelist} 
    ...
    for site in sitelist:
        ...
        t = multiprocessing.Process(None, blogViewer.blogViewer, site,
              (site,
               curlLock, 
               urls, 
               '%s/%s.%s.%s' % 
                  (directory, site.replace('/', '_'), start_month, start_year)
              )
            )
        t.url = requests[site]
        t.start()
...
if len(sys.argv) > 1:
  ...
  queries = (
             ['Israel+Iran+Bombing', 01, 2012, 02, 2012],
            )
