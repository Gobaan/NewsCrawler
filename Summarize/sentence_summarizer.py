...
with open('models/%s.matched.sentences' % target_filename, 'r') as infile:
  target_model = json.load(infile)

try:
  with open('models/background.model.sentences', 'r') as infile:
  ...
  for name in files:
    ...
    for doc_match in doc_regex.finditer(data):
      ...
      for comment_match in comment_regex.finditer(document): 
        ...
        if selector:
          our_comments += 1
          target_comments += [sentence for sentence in
            nltk.sent_tokenize(comment)]
        else:
          their_comments += 1
...
def get_terms(sentence):
  ...
  terms = set([word for word in nltk.word_tokenize(sentence)])
